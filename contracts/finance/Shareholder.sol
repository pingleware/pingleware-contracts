...
library Shareholder {
    ...
    struct ShareholderStorage {
        mapping(address => address[]) _shareholders; // all sharenolders by contract offering address
        mapping(address => address) cancellations;
        mapping(address => uint256) holderIndices;
        mapping(address => string)  verified;
        address[] _verified;
        address[] shareholders;
        address[] shareholders_accredited;
        address[] shareholders_nonacredited;
        address[] shareholders_affiliate;
        address[] shareholders_sophisticated;
        mapping(address => uint256) maxNonaccredited; // mapped to offering contract address; -1=unlimited
    }

    function shareholderStorage()
        internal
        pure
        returns (ShareholderStorage storage ds)
    {
    ...
    function isActiveShareholder(address addr)
        internal
        view
        returns (bool)
    {
        ...
        returns (bool)
    {
        require(addr != ZERO_ADDRESS,"invalid address");
        bool found = false;
        uint256 length = shareholderStorage().shareholders.length;
        for (uint256 i = 0; i < length; i++) {
            if (shareholderStorage().shareholders[i] == addr) {
                found = true;
            }
        }
        return found;
    ...
    function isAccredited(address addr)
        internal
        view
        returns (bool)
    {
        require(addr != ZERO_ADDRESS,"invalid address");
        bool found = false;
        uint256 length = shareholderStorage().shareholders_accredited.length;
        for (uint256 i = 0; i < length; i++) {
            if (shareholderStorage().shareholders_accredited[i] == addr) {
                found = true;
            }
        }
        return found;
    ...
    function isNonAccredited(address addr)
        internal
        view
        returns (bool)
    {
        require(addr != ZERO_ADDRESS,"invalid address");
        bool found = false;
        uint256 length = shareholderStorage().shareholders_nonacredited.length;
        for (uint256 i = 0; i < length; i++) {
            if (shareholderStorage().shareholders_nonacredited[i] != addr) {
                found = true;
            }
        }
        return found;
    ...
    function isAffiliate(address addr)
        internal
        view
        returns (bool)
    {
        require(addr != ZERO_ADDRESS,"invalid address");
        bool found = false;
        uint256 length = shareholderStorage().shareholders_affiliate.length;
        for (uint256 i = 0; i < length; i++) {
            if (shareholderStorage().shareholders_affiliate[i] != addr) {
                found = true;
            }
        }
        return found;
    ...
    function isSophisticated(address addr)
        internal
        view
        returns (bool)
    {
        require(addr != ZERO_ADDRESS,"invalid address");
        bool found = false;
        uint256 length = shareholderStorage().shareholders_sophisticated.length;
        for (uint256 i = 0; i < length; i++) {
            if (shareholderStorage().shareholders_sophisticated[i] != addr) {
                found = true;
            }
        }
        return found;
    ...
    function setMaxUnaccreditInvestors(address offering, uint256 value)
        external
    {
        require(value >= 0,"cannot be negative");
        shareholderStorage().maxNonaccredited[offering] = value;
    }
     ...
     * 
     */
    function addShareholder(address offering, address addr, InvestorType level)
        external
    {
        require(addr != ZERO_ADDRESS,"invalid address");
        if (shareholderStorage().holderIndices[addr] == 0) {
            if (level == InvestorType.accredited) {
                require(shareholderStorage().shareholders_nonacredited.length < shareholderStorage().maxNonaccredited[offering], "will exceed the maximum number of non-accredited investors");
                shareholderStorage().shareholders_nonacredited.push(addr);
                shareholderStorage().holderIndices[addr] = 1;
            } else if (level == InvestorType.affiliate) {
              shareholderStorage().shareholders.push(addr);
              shareholderStorage().holderIndices[addr] = 1;
            } else if (level == InvestorType.nonaccredited) {
              shareholderStorage().shareholders_affiliate.push(addr);
              shareholderStorage().holderIndices[addr] = 1;
            }
            shareholderStorage()._shareholders[offering].push(addr);
        }
    }
    ...
    {
        require(original != ZERO_ADDRESS,"invalid address");
        require(replacement != ZERO_ADDRESS,"invalid address");
