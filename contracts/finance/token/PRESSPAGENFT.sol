...
contract PressPageNFT {
    ...
    string private constant _name = "PressPage Entertainment Inc Commodity Token";
    string private constant _symbol = "PRESS.NFT";
    uint256 private constant _totalSupply = 200000;
    ...
    /// Self Destruct
    function destroy() external isContractOwner {
    ...
    /// Modifier to check whitelisted wallets
    modifier isWhitelisted(address _address) {
        require(_whitelisted[_address], "investor wallet not authorized");
        _;
    }
    ...
    function addWhitelisted(address _address) external isContractOwner {
        require(_whitelisted[_address] == false, "wallet is already whitelisted");
    ...
    function _mint(uint256 tokenID) external isContractOwner {
        ...
        require(_balances[_contractOwner] < _totalSupply, "Maximum tokens have been minted");
    ...
    function _burn(uint256 tokenID, uint256 count) external isContractOwner {
        address owner = _ownerOf[tokenID];
        if (owner == address(0)) {
            return;
        }
        require(count < _balances[owner], "All tokens have been burnt");
        _balances[owner] -= count;
        _ownerOf[tokenID] = address(this); // place in treasury shares
        _approvals[tokenID] = address(this); // place in treasury shares
        /// Emit event
    ...
    function transferFrom(address from, address to, uint256 tokenID) external payable isWhitelisted(from) isWhitelisted(to) {
    ...
    function safeTransferFrom(address from, address to, uint256 tokenID, bytes memory data)
        external payable isWhitelisted(from) isWhitelisted(to) {
        _doTransferFrom(from, to, tokenID);
    ...
    /// @param to The new owner
    /// @param tokenID The NFT to transfer
    function safeTransferFrom(address from, address to, uint256 tokenID) external payable isWhitelisted(from) isWhitelisted(to) {
        _doTransferFrom(from, to, tokenID);
