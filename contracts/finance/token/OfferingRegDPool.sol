...
import "./OfferingRegD.sol";

contract OfferingRegDPool {
    ...
    function createToken(address issuer, string memory name, string memory symbol, uint tokens, uint price, address catContractAddress, int maxNonAccreditedInvestors) public isOwner {
        ...
        address newToken = address(new OfferingRegD(owner, issuer, name, symbol, tokens, catContractAddress, maxNonAccreditedInvestors));
    ...
    function addTransferAgent(string memory symbol,address transferAgent) public isOwner {
        ...
        OfferingRegD token = OfferingRegD(tokenContracts[symbol]);
    ...
    function matchOrders(string calldata _currencyPair) internal {
        ...
        while (i < buys.length && j < sells.length) {
            ...
            if (buyOrder.price >= sellOrder.price) {
                ...
                IOfferingRegD token = IOfferingRegD(tokenContracts[_currencyPair]);
